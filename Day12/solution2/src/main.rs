...
fn main() {
    ...
    println!("{:?}", path.iter().map(|x| x.len()).collect::<Vec<usize>>());
}

fn pathfind(mut map: Vec<Vec<char>>) -> Vec<Vec<(usize, usize)>> {
    ...
    for y in 0..HEIGHT {
        'outer: for x in 0..WIDTH {
            ...
            if map[y][x] == 'a' {
                ...
                while current != local_start {
                    path.push(current);    
                    // println!("{local_start:?} -> {current:?} -> route: {:?}", table[&current].route);
                    // current = table[&current].route.unwrap();
                    current = match table[&current].route {
                        Some(x) => x,
                        None    => continue 'outer,
                    };
                }
